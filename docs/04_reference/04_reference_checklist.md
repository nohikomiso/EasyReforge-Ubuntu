...
### 160+ 個別スクリプト生成（完了済み・保守用）

- [x] **Task 4.1**: スクリプト生成ツール実装
  - [x] メタデータCSV読み込み
  - [x] テンプレートエンジン実装
  - [x] 154+ スクリプト生成（個別更新用）
  - [x] 生成スクリプトファイルへの書き込み

### データ駆動型ダウンロード・オーケストレーション（新方針）

- [x] **Task 4.2**: CSV一括ダウンロードエンジンの実装
  - [x] `Download/lib/download_engine.py` (または .sh) 実装
  - [x] CSV パースおよびフィルタリング（--type, --tag）ロジック
  - [x] `DRY_RUN` 環境変数への対応
  - [x] エラーハンドリングと進捗ログ出力

- [x] **Task 4.3**: メタデータへのバリアント（Minimum/Standard等）タグ付与
  - [x] `Download/metadata.csv` への `tags` カラム追加
  - [x] 既存バッチ（`NoobAiCommon_Minimum.bat`等）の解析とタグ紐付け

- [x] **Task 4.4**: メタスクリプト（All...）の再実装
  - [x] `Download/All/All*.sh` をエンジン呼び出しのラッパーとして作成
  - [x] `AllModels_Minimum.sh` 等のバリアント用スクリプト作成

### 検証と最適化

- [x] **Task 4.5**: 全スクリプトの Shellcheck 検証
  - [x] エンジンおよびラッパースクリプトの検証
  - [x] エラー修正と合格確認

- [x] **Task 4.6**: E2E ドライランテスト
