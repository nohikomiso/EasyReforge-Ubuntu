...
# - docs/../04_reference/04_reference_conversion_table.md (syntax lookup only)
...
### 🌍 環境変数カスケードパターン (Configuration Cascade Pattern)

**【最重要アーキテクチャ概念】**
このプロジェクトの根本的な思想として「**起点のインストーラー スクリプト（`easyreforge_installer.sh`等）一つを単独の空フォルダにダウンロード実行するだけで、すべてが完了する**」という設計が存在します。

そのため、起点スクリプトで定義された設定値は、後続でクローン・実行されるすべての子スクリプトへ暗黙的に引き継がれる必要があります（Windowsバッチにおける `set` の特性）。
これをUbuntuで再現するため、以下のルールを厳守してください：

1. **親スクリプトは設定を `export` すること**
   起点となるスクリプトで定義する共通変数は `readonly` に留めず、必ず `export` で環境変数として公開してください。
   ```bash
   export PROJECT_NAME="EasyReforge"
   export PROJECT_URL="..."
   ```

2. **子スクリプトはフォールバック評価で受け取ること**
   下流のスクリプト（`setup.sh`、`reforge.sh` 等）では、親からの設定注入を前提としつつ、単独実行時にも備えて `${VAR:-default}` の形式で受け取ってください。
   ```bash
   PROJECT_NAME="${PROJECT_NAME:-EasyReforge}"
   ```
   **警告**: 未使用だと思ってローカル変数を勝手に消したり、`export` 指定を外したりすると「ダウンストリームで設定が引き継がれない」バグが生じます。Shellcheck の一時的な警告（SC2034 等）には、不用意に削るのではなく `export` などのアーキテクチャに沿った対処をしてください。
